...
public static int[][] readData( String fileName) {
  ...
  try {
    ...
    StringTokenizer st;
    //Create a tokenizer to break each line into numbers that can be put as integers in our data array
    ...
    //Close and then reopen the scanner to reset its cursor at the start of the file
    
    st = new StringTokenizer(fileReader.nextLine());
    cols = st.countTokens();
    //Tokenize the first line (since each line has the same length it does not matter which one we use) and then count the number of values in the line, that is our column length
    
    data = new int[rows][cols];
    //Initialize the integer array with the lengths we just found
    ...
    for (int i = 0; i < rows; i++){
      ...
      if (i != rows - 1){
        //As long as we are not at the last line in the file we want to tokenize the next line, if we did not have this the program would attempt to tokenize a non-existant
        //line after the final line in the file
        st = new StringTokenizer(fileReader.nextLine());
      } 
    }
    fileReader.close();
