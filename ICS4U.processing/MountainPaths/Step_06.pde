...
public  int indexOfLowestElevPath( int[][] data ) {
  ...
  //Set the best elevation change to the first rows paths elevation change, set the bestRow to this first row
  ...
  //fill the rectangle with red
    ...
    for (int row = 1; row < data.length ; row++){
      ...
      //Set the next elevation change to the next rows elevation change
      
      if (bestElevChange > nextElevChange){
        //If the next elevation change is less than the best elevation change, set a new bestElevChange and set the best row to the current row
  ...
  return bestRow;
  //Return the best row (to be drawn in Task 7)
