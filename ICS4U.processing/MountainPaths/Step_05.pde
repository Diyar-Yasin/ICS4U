...
public int drawLowestElevPath( int[][] data, int row, boolean bestPath ) {
  ...
  int opt1, opt2, opt3, minOpt, elevChange = 0;                    //3 options, with at least one of them being a minimum and the elevation change 
                                                                   //  of the path we take
  noStroke();                                                      //Do not outline the rectangle (as the rectange will be 1x1 and you will not be able 
                                                                   //  to see the color of the center of the rectangle)
  if (bestPath){
  ...
  rect(0, row, 1, 1);                                             //Draw the first rectangle at x = 0 and the row (y), 1x1 as usual
  ...
  for(int col = 0; col != data[row].length - 1; col++){ 
    ...
    if (row != 0 && row != data.length - 1){
      ...
      opt1 = Math.abs(data[row+1][col+1] - data[row][col]);       //Option 1 is the bottom right path
      opt2 = Math.abs(data[row][col+1] - data[row][col]);         //Option 2 is the middle right path
      opt3 = Math.abs(data[row-1][col+1] - data[row][col]);       //Option 3 is the top right path

      
      
      minOpt = min(opt1, opt2, opt3);                             //Find the minimum elevation change from our current position to each of the 3 options 
                                                                  //and set it to our minOpt
    }
    else if (row == 0){
      ...
      minOpt = min(opt2, opt1);                                    //Find the minimum between the possible options
      opt3 = minOpt + 1;                                           //Set the impossible option to the minimum value + 1, so that when we use if statements 
                                                                   //  to check which opt is equal to the minOpt it will never be this
    }
    else {
      ...
      minOpt = min(opt3, opt2);                                    //Find the minimum between the possible options
      opt1 = minOpt + 1;                                           //Set the impossible option to the minimum value + 1, so that when we use if statements 
                                                                   //  to check which opt is equal to the minOpt it will never be this
    }
    ...
    if (minOpt == opt2){
      ...
      elevChange = elevChange + opt2;                              //Add the elevation change of option 2 to elevChange
      rect(col+1, row, 1, 1);                                      //Draw a rectangle at the same y value and the x value to the right of where we are
    }
    else if (opt1 == opt3){
      ...
      if ((int) Math.round(Math.random()) == 1){
        ...
        elevChange = elevChange + opt1;                            //Add to our elevation
        row++;                                                     //Going down a row (by adding 1 to our row, ie y value)
        rect(col+1, row, 1, 1);                                    //Draw a rectangle at that y value and the x value to the right of where we are
      }
      else {
        elevChange = elevChange + opt3;                            //Add to our elevation
        row--;                                                     //Going up a row (by subtracting 1 from our row, ie y value)
        rect(col+1, row, 1, 1);                                    //Draw a rectangle at that y value and the x value to the right of where we are
      }
    }
    else if (minOpt == opt1){
      //If the middle path is not the minimum and the top andd bottom path are not the same (so they are not both the minimum) we ask if option 1 is the minimum
  ...
  return elevChange;                                               //Return our elevation change after drawing the entire line 
}
